    ...
    implements OnInit
{
    ...
    editarUsuario(usuario: Usuario) {
        ...
        this.routingService.salvarValor(
            RotaVoltarParameter,
            TabelaUsuarioRoute
        );
        this.router.navigate([
            { outlets: { secondRouter: FormularioUsuarioRoute } },
        ]);
    }

    excluirUsuario(usuario: Usuario) {
        this.usuarioService.deletar(usuario.id).subscribe((data) => {
            if (data) {
                this.pesquisar();
            }
            this.list = data.map((x) => Object.assign(new Usuario(), x));
