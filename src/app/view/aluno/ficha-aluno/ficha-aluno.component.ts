...
export class FichaAlunoComponent implements OnInit {
    ...
    @ViewChild("turmaCrachaTemplate", { static: true })
    turmaCrachaTemplate: TemplateRef<any>;
    ...
    constructor(
        ...
        this.columnsTurma.push({
            key: "expand",
            groupKey: "table",
            bodyTemplateName: "expand",
        } as Coluna);
        this.columnsTurma.push({
            key: "curso",
            header: "Curso",
            groupKey: "table",
            field: "turma.curso.nome",
        } as Coluna);
        this.columnsTurma.push({
            key: "turma",
            header: "Turma",
        ...
        this.columnsTurma.push({
            key: "expandedDetail",
            classBody: "rowexpansion",
            colspan: 7,
            groupKey: "expandGroupping",
            bodyTemplateName: "expandedDetailTemplate",
        } as Coluna);
    ...
    ngOnInit(): void {
        if (this.routingService.possuiValor(IdAlunoParameter)) {
            this.idAluno = this.routingService.excluirValor(
                IdAlunoParameter
            ) as number;
            this.rotaVoltar =
                this.routingService.excluirValor(RotaVoltarParameter);
            this.carregarAluno();
    ...
    voltar() {
        if (this.rotaVoltar == HomeRoute) {
            this.router
                .navigate([{ outlets: { secondRouter: null } }])
                .then(() => this.router.navigate([this.rotaVoltar]));
        } else {
            this.router.navigate([
                { outlets: { secondRouter: this.rotaVoltar } },
            ]);
        }
    }
    ...
    vincularTurma() {
        const dialogRef = this.dialog.open(ModalConfirmacaoComponent, {
            data: {
                mensagem: `Deseja vincular o aluno ${this.element.nome} em uma turma?`,
            },
        });
        dialogRef.afterClosed().subscribe((result) => {
            if (result) {
                this.dialog
                    .open(TurmaAlunoComponent, { data: this.element })
                    .afterClosed()
                    .subscribe((res) => {
                        this.carregarAluno();
                    });
    ...
    cracha() {
        ...
        if (this.element.turmasAluno.length > 1) {
            ...
            dialogRef = this.dialog.open(ModalConfirmacaoComponent, {
                data: {
                    mensagem: `Deseja gerar o crachá do aluno ${this.element.nome} para qual turma?`,
                    template: this.turmaCrachaTemplate,
                },
            });
        } else {
            dialogRef = this.dialog.open(ModalConfirmacaoComponent, {
                data: {
                    mensagem: `Deseja gerar o crachá do aluno ${this.element.nome}?`,
                },
            });
        }
        dialogRef.afterClosed().subscribe((result) => {
            if (result) {
                this.alunoService
                    .gerarCracha(this.turmaCrachaSelecionada.id)
                    .subscribe((data) => {
                        if (data) {
    ...
    transferirTurma() {
        const dialogRef = this.dialog.open(ModalConfirmacaoComponent, {
            data: {
                mensagem: `Deseja transferir o aluno ${this.element.nome} de turma?`,
            },
        });
        dialogRef.afterClosed().subscribe((result) => {
            if (result) {
                this.dialog
                    .open(TransferenciaAlunoComponent, { data: this.element })
                    .afterClosed()
                    .subscribe((res) => {
                        this.carregarAluno();
                    });
    ...
    alterarSituacao() {
        const dialogRef = this.dialog.open(ModalConfirmacaoComponent, {
            data: {
                mensagem: `Deseja alterar a situação do aluno ${this.element.nome}?`,
            },
        });
        dialogRef.afterClosed().subscribe((result) => {
            if (result) {
                this.dialog
                    .open(AlteracaoSituacaoComponent, { data: this.element })
                    .afterClosed()
                    .subscribe((res) => {
                        this.carregarAluno();
                    });
    ...
    gerarHistorico(turmaAluno: TurmaAluno) {
        const dialogRef = this.dialog.open(ModalConfirmacaoComponent, {
            data: {
                mensagem: `Deseja gerar o histórico escolar de ${turmaAluno.turma.curso.nome} do aluno ${this.element.nome}?`,
            },
        });
        dialogRef.afterClosed().subscribe((result) => {
            if (result) {
                this.alunoService
                    .gerarHistorico(turmaAluno.id)
                    .subscribe((data) => {
                        if (data) {
                            this.baixarArquivoService.downloadFile(
                                data,
                                `${this.element.nome}.docx`,
    ...
    editarAluno() {
        ...
        this.router.navigate([
            { outlets: { secondRouter: FormularioAlunoRoute } },
        ]);
    }
    ...
    expandTable(element: TurmaAluno) {
        ...
        if (this.expandedTurma.includes(element)) {
            this.expandedTurma = this.expandedTurma.filter(
                (x) => x !== element
            );
            ...
            if (element.notas == null || element.notas.length == 0) {
                forkJoin([
                    this.disciplinaService.listarDisciplinasDeUmCurso(
                        element.turma.curso.id
                    ),
                    ...
                    disciplinas.forEach((d) => {
                        ...
                        if (nota == null) {
                            ...
                            nota.disciplina = d;
                            nota.valorNota = parseFloat(
                                nota.valorNota.toString().replace(",", ".")
                            );
                        }
