...
export class AlteracaoSituacaoComponent
    extends BaseFormularioComponent<MudancaSituacao>
    implements OnInit
{
    ...
    ngOnInit(): void {
        ...
        this.turmasVinculadas = this.data.turmasAluno.filter(
            (x) => !this.turmaConcluida(x)
        );
        if (this.turmasVinculadas.length == 0) {
            this.notificationService.addNotification(
                "Atenção",
                "O aluno selecionado não possui turmas disponíveis para mudança de situação.",
    ...
    closeModal(salvar: boolean) {
        if (salvar) {
            if (this.validar()) {
                ...
                this.element.situacaoId =
                    this.tiposStatusAlunoSelecionado.value;
                if (!this.ehConclusao()) {
                    this.element.codigoSistec = null;
                    this.element.dataConclusao = null;
                }
                this.alunoService
                    .alterarSituacao(this.element)
                    .subscribe((data) => {
                        if (data) {
                            this.notificationService.addNotification(
                                "Sucesso!",
                                "Situação do aluno alterada com sucesso.",
    ...
    buscarStatus() {
        ...
        switch (this.turmaAtualSelecionada.tipoStatusAluno) {
            ...
            case TipoStatusAlunoEnum.Trancado.name: {
                this.tiposStatusAlunoOptions = [TipoStatusAlunoEnum.Ativo];
                break;
            }
    ...
    ehConclusao() {
        return (
            this.tiposStatusAlunoSelecionado == TipoStatusAlunoEnum.Concluido
        );
    }
    ...
    validar(): boolean {
        ...
        if (this.turmaAtualSelecionada == null) {
            ...
            this.notificationService.addNotification(
                "Erro!",
                "Selecione uma turma que o aluno esteja vinculado.",
        ...
        if (this.tiposStatusAlunoSelecionado == null) {
            ...
            this.notificationService.addNotification(
                "Erro!",
                "Selecione um status novo para o aluno.",
                NotificationType.Error
            );
        }
